...
### Cerebras WSE向けGNN実装
ウェハスケールプロセッサ上でGNN学習パイプラインを実装しました。学部卒業研究から継続して性能分析を行っています。
...
### 大規模AIアクセラレータ上の機械学習の性能分析
ウェハスケールプロセッサを含む大規模AIアクセラレータ上で、実行特性の測定とボトルネック分析に関心があります。

### MLIRを利用するプログラミング言語
MLIRを基盤に、高水準言語からWSE向けカーネルを生成するコンパイラの開発を目指しています。

### AI Safety
高度なAIがもたらす重大な危険を抑えるAI Safetyに関心があります。AI for Scienceと研究開発の自動化を見据え、異種AIアクセラレータ上の計算とAIが操作する研究環境を検証し、危険な実行を制限・停止できる仕組みを研究したいと考えています。
